...
!
        IF(HL(1).LT.HDIS.OR.HDIS.EQ.0.) GO TO 40
...
!
        ...
        VHGT = AMAX1(0.,VHGT)
        AMASS2 = AACC* (HL(1)*RO + VHGT/V)
!       WRITE(6,1234) Z1,Z2,DIFF1,DIFF2,AMASS1,AMASS2,RMT,RMSL
!1234   FORMAT(8(1PE12.3))
!
!       SKIP OUT IF LEVEL WILL NOT RISE ABOVE SECOND HOLE
!
        IF(IHOLE.EQ.1) GO TO 40
!
!       CHECK FOR CONVERGENCE ON LEVEL ABOVE SECOND HOLE
!
        IF(ABS(AMASS1-AMASS2).LT.CNVACC) GO TO 40
        SLOPE = (Z1-Z2)/(DIFF1-DIFF2)
        IF(ABS(DIFF1).LT.ABS(DIFF2)) GO TO 20
        AMASS1 = AMASS2
        DIFF1 = DIFF2
        Z1 = Z2
20      Z2 = Z1 - DIFF1*SLOPE
        Z2 = AMAX1(Z2,HDIS)
30      CONTINUE
!
!       PRINT RESULTS
!
        ...
        WRITE(6,602) ERROR
40      CONTINUE
!        IF(VHGT.EQ.0.) WRITE(6,600)
        ACCMAS = AMASS2
100     XLEVEL = HL(1)*12.
201     CONTINUE
!       WRITE(6,604) ACCMAS,XLEVEL
!
600     FORMAT(' ACCUMULATOR IS FULL')
602     FORMAT(' ACCUML DOES NOT CONVERGE, MAX.ERROR =',1PE10.3,' LBM')
!604    FORMAT(/'  ACCMAS = ',F8.3,' LBM',3X,'LIQUID LEVEL = ',F8.2,
!     &          ' IN')
!

...
! Somethings these small silly things are great to grab on to when starting up with the code after being off for a while
    ...
    !CHARACTER*150 LineData !One-line data in circuit file  !RS: Debugging: Extraneous
    ...
    !INTEGER NumSection !Loop counter, ISI - 09/10/0 !RS: Debugging: Brought over from Evaporator file  !RS: Debugging: Extraneous
    !INTEGER NumInletSections !Number of inlet sections, ISI - 09/10/07  !RS: Debugging: Brought over from Evaporator file  !RS: Debugging: Extraneous
...
IF (CoilType .EQ. CONDENSERCOIL) THEN !Fin-tube coil
    ...
    !!IDC ckt    !RS: Debugging: Brought over from the Evaporator IDC case---trying to get the program to properly handle heating AND cooling
    !
    !    !**************************** Geometry *************************************
    !
    !    !Reading in the variable values

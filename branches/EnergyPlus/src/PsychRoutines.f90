...
#ifdef EP_psych_errors
  USE General !, ONLY: TrimSigDigits    !RS: Debugging: 102612
#endif
...
PRIVATE ! Everything private unless explicitly made public
      ...
      INTEGER, PARAMETER :: iPsyRhFnTdbRhovLBnC =13
      ...
      REAL, PARAMETER :: KelvinConv=273.15
#endif
...
PUBLIC  PsyRhovFnTdbRhLBnC
PUBLIC  PsyRhovFnTdbWPb
PUBLIC  PsyRhFnTdbRhov
PUBLIC  PsyRhFnTdbRhovLBnC
...
function PsyHfgAirFnWTdb(w,T,calledfrom) result(hfg)
  ...
  IMPLICIT NONE    ! Enforce explicit typing of all variables in this routine
          ...
          ! FUNCTION ARGUMENT DEFINITIONS:
      REAL, intent(in)  :: w    ! humidity ratio {kgWater/kgDryAir} !unused1208
      REAL, intent(in)  :: T    ! input temperature {Celsius}
      character(len=*), intent(in), optional :: calledfrom  ! routine this function was called from (error messages) !unused1208
      REAL         :: hfg  ! result => heat of vaporization for moist air {J/kg}
          ...
          ! FUNCTION LOCAL VARIABLE DECLARATIONS:
      REAL :: hg  ! enthalpy of the gas
      REAL :: hf  ! enthalpy of the fluid
!      INTEGER,SAVE :: b0cerrcount=0
      REAL :: Temperature    ! input temperature {Celsius} - corrected for >= 0C
...
function PsyHgAirFnWTdb(w,T,calledfrom) result(hg)
  ...
  IMPLICIT NONE    ! Enforce explicit typing of all variables in this routine
      ...
      REAL, intent(in)  :: T   ! input temperature {Celsius}
      character(len=*), intent(in), optional :: calledfrom  ! routine this function was called from (error messages) !unused1208
      REAL         :: hg  ! enthalpy of the gas {units?}
...
FUNCTION PsyTdpFnTdbTwbPb(TDB,TWB,PB,calledfrom) RESULT(TDP)
  ...
  IMPLICIT NONE    ! Enforce explicit typing of all variables in this routine
      ...
      REAL, intent(in)  :: TWB  ! wet-bulb temperature {C}
      REAL, intent(in)  :: PB   ! barometric pressure (N/M**2) {Pascals}
      character(len=*), intent(in), optional :: calledfrom  ! routine this function was called from (error messages)
      REAL         :: TDP  ! result=> dew-point temperature {C}
...
FUNCTION PsyTdpFnWPb(W,PB,calledfrom) RESULT(TDP)
  ...
  IMPLICIT NONE    ! Enforce explicit typing of all variables in this routine
      ...
      REAL         :: TDP  ! result=> dew-point temperature {C}
          ...
          ! FUNCTION LOCAL VARIABLE DECLARATIONS:
      REAL PDEW  ! pressure at dew point temperature
      REAL W0    ! limited humidity ratio
...
FUNCTION PsyRhovFnTdbRh(Tdb,RH,calledfrom) RESULT(RhoV)
  ...
  IMPLICIT NONE    ! Enforce explicit typing of all variables in this routine
      ...
      REAL, intent(in) :: Tdb    ! dry-bulb temperature {C}
      character(len=*), intent(in), optional :: calledfrom  ! routine this function was called from (error messages) !unused1208
      REAL        :: RhoV   ! Vapor density in air
          ...
          ! FUNCTION LOCAL VARIABLE DECLARATIONS:
        REAL :: pws  ! saturation pressure for Tdb
...
FUNCTION PsyRhovFnTdbRhLBnC(Tdb,RH,calledfrom) RESULT(RhoV)
  ...
  IMPLICIT NONE    ! Enforce explicit typing of all variables in this routine
      ...
      REAL        :: RhoV   ! Vapor density in air
          ...
          ! FUNCTION LOCAL VARIABLE DECLARATIONS:
      RhoV = RH/(461.52*(Tdb+KelvinConv))*Exp(23.7093-4111.0/ &
                ((Tdb+KelvinConv)-35.45))

  return
end function PsyRhovFnTdbRhLBnC
...
FUNCTION PsyRhFnTdbRhov(Tdb,Rhovapor,calledfrom) RESULT(RHValue)
  ...
  IMPLICIT NONE    ! Enforce explicit typing of all variables in this routine
          ...
          ! FUNCTION ARGUMENT DEFINITIONS:
      REAL, intent(in) :: Tdb     ! dry-bulb temperature {C}
      REAL, intent(in) :: Rhovapor  ! vapor density in air {kg/m3}
      character(len=*), intent(in), optional :: calledfrom  ! routine this function was called from (error messages)
          ...
          IF (RHValue > 1.01) THEN
            IF (.not. WarmupFlag) THEN
              ...
              CALL ShowRecurringWarningErrorAtEnd('Calculated Relative Humidity out of range (PsyRhFnTdbRhov)',   &
                 iPsyErrIndex(iPsyRhFnTdbRhov),ReportMinOf=RHValue*100,ReportMaxOf=RHValue*100,  &
...
FUNCTION PsyRhFnTdbRhovLBnC(Tdb,Rhovapor,calledfrom) RESULT(RHValue)
...
!      integer,save :: b0cerrcount=0
      ...
      If(Rhovapor <= 0.0) Then
        RHValue = 0.0
      Else
        RHValue =Rhovapor*461.52*(Tdb+KelvinConv)*Exp(-23.7093+4111.0/ &
                ((Tdb+KelvinConv)-35.45))
      End If

#ifdef EP_psych_stats
      NumTimesCalled(iPsyRhFnTdbRhovLBnC)=NumTimesCalled(iPsyRhFnTdbRhovLBnC)+1
#endif
...
!                   VALIDITY TEST
      IF (RHValue < 0.0 .or. RHValue > 1.0) THEN
        IF (RHValue > 1.0) THEN
          ...
          IF (RHValue > 1.01) THEN
            IF (.not. WarmupFlag) THEN
              IF (iPsyErrIndex(iPsyRhFnTdbRhovLBnC) == 0) THEN
                String=' Dry-Bulb= '//TRIM(TrimSigDigits(TDB,2))//  &
                       ' Rhovapor= '//TRIM(TrimSigDigits(Rhovapor,3))// &
                       ' Calculated Relative Humidity [%]= '//TRIM(TrimSigDigits(RHValue*100.,2))
                CALL ShowWarningMessage('Calculated Relative Humidity out of range (PsyRhFnTdbRhovLBnC) ')
                if (present(calledfrom)) then
                  CALL ShowContinueErrorTimeStamp(' Routine='//trim(calledfrom)//',')
                else
                  CALL ShowContinueErrorTimeStamp(' Routine=Unknown,')
                ...
                CALL ShowContinueError('Relative Humidity being reset to 100.0%')
              ENDIF
              CALL ShowRecurringWarningErrorAtEnd('Calculated Relative Humidity out of range (PsyRhFnTdbRhovLBnC)',   &
                 iPsyErrIndex(iPsyRhFnTdbRhovLBnC),ReportMinOf=RHValue*100.,ReportMaxOf=RHValue*100.,  &
                 ReportMinUnits='%',ReportMaxUnits='%')
            ENDIF
          ENDIF
#endif
      ...
      ENDIF    ! RHValue in proper range
 return
end function PsyRhFnTdbRhovLBnC
            ...
            IF (RHValue > 1.01) THEN
              IF (.not. WarmupFlag) THEN
                ...
                CALL ShowRecurringWarningErrorAtEnd('Calculated Relative Humidity out of range (PsyRhFnTdbWPb)',   &
                  iPsyErrIndex(iPsyRhFnTdbWPb),ReportMinOf=RHValue*100.,ReportMaxOf=RHValue*100.,  &
                  ReportMinUnits='%',ReportMaxUnits='%')
              ENDIF
            ENDIF
#endif
        ...
        IF (RHValue < 0.0 .or. RHValue > 1.0) THEN
          IF (RHValue > 1.0) THEN
            ...
            IF (RHValue > 1.01) THEN
              IF (.not. WarmupFlag) THEN
                IF (iPsyErrIndex(iPsyRhFnTdbWPb) == 0) THEN
                  String=' Dry-Bulb= '//TRIM(TrimSigDigits(TDB,2))//  &   !RS: Debugging: 102612
                         ' Humidity Ratio= '//TRIM(TrimSigDigits(W,3))// &    !RS: Debugging: 102612
                         ' Calculated Relative Humidity [%]= '//TRIM(TrimSigDigits(RHValue*100,2)) !TrimSigDigits(RHValue*100.,2)) !RS: Debigging: 102612
                  CALL ShowWarningMessage('Calculated Relative Humidity out of range (PsyRhFnTdbWPb) ')
                  if (present(calledfrom)) then
                    CALL ShowContinueErrorTimeStamp(' Routine='//trim(calledfrom)//',')
                  else
...
FUNCTION PsyTwbFnTdbWPb(TDB,dW,Patm,calledfrom) RESULT(TWB)
  ...
  IMPLICIT NONE    ! Enforce explicit typing of all variables in this routine
      ...
      REAL, intent(in) :: TDB    ! dry-bulb temperature {C}
      REAL, intent(in) :: dW      ! humidity ratio
      REAL, intent(in) :: Patm   ! barometric pressure {Pascals}
      character(len=*), intent(in), optional :: calledfrom  ! routine this function was called from (error messages)
      REAL        :: TWB    ! result=> Temperature Wet-Bulb {C}
      ...
      REAL :: convTol=0.0001
          ...
          ! FUNCTION LOCAL VARIABLE DECLARATIONS:
      REAL:: tBoil        ! Boiling temperature of water at given pressure
      REAL, SAVE  :: last_Patm=-99999.   ! barometric pressure {Pascals}  (last)
      REAL, SAVE  :: last_tBoil=-99999.       ! Boiling temperature of water at given pressure (last)
      REAL:: newW         ! Humidity ratio calculated with wet bulb guess
      REAL:: W            ! Humidity ratio entered and corrected as necessary
      REAL:: ResultX      ! ResultX is the final Iteration result passed back to the calling routine
      REAL:: WBT          ! Current Value of WetBulbTemperature
      REAL:: error        ! Deviation of dependent variable in iteration
      REAL:: X1           ! Independent variable in ITERATE
      REAL:: Y1           ! Dependent variable in ITERATE
      REAL:: Wstar        !Humidity  ratio as a function of Sat Press of Wet Bulb
      REAL:: PSatstar     !Saturation pressure at wet bulb temperature
      ...
      FlagError=.false.
#ifdef EP_psych_errors
      IF (TDB <= -100.0 .or. TDB >= 200.0) THEN
        IF (.not. WarmupFlag) THEN
      ...
      DO iter = 1,itmax
        ...
        IF (WBT .GE. (tBoil-0.09) ) WBT = tBoil-0.1
        ...
        newW=((2501.0-2.381*WBT)*Wstar-(TDB-WBT)) /          &
          (2501.0+1.805*TDB-4.186*WBT)
        ...
        IF((PSatstar > 611.000) .and. (PSatstar < 611.25) .and. (abs(error) <= 0.0001) .and. (iter > 4)) Exit
...
FUNCTION PsyVFnTdbWPb(TDB,dW,PB,calledfrom) RESULT(V)
  ...
  IMPLICIT NONE    ! Enforce explicit typing of all variables in this routine
      ...
      REAL, intent(in) :: PB     ! barometric pressure {Pascals}
      character(len=*), intent(in), optional :: calledfrom  ! routine this function was called from (error messages)
      REAL        :: V      ! result=> specific volume {m3/kg}
      ...
      w=MAX(dW,1.0*10**-5)
      V=1.59473*10**2*(1.+1.6078*W)*(1.8*TDB+492.)/PB   !RS: Debugging: 102612
...
FUNCTION PsyWFnTdpPb(TDP,PB,calledfrom) RESULT(W)
  ...
  IMPLICIT NONE    ! Enforce explicit typing of all variables in this routine
          ...
          ! FUNCTION ARGUMENT DEFINITIONS:
      REAL, intent(in) :: TDP    ! dew-point temperature {C}
      REAL, intent(in) :: PB     ! barometric pressure {Pascals}
      character(len=*), intent(in), optional :: calledfrom  ! routine this function was called from (error messages)
      REAL        :: W      ! result=> humidity ratio
          ...
          ! FUNCTION LOCAL VARIABLE DECLARATIONS:
      REAL PDEW ! saturation pressure at dew-point temperature {Pascals}
...
FUNCTION PsyWFnTdbTwbPb(TDB,TWBin,PB,calledfrom) RESULT(W)
  ...
  IMPLICIT NONE    ! Enforce explicit typing of all variables in this routine
          ...
          ! FUNCTION ARGUMENT DEFINITIONS:
      REAL, intent(in) :: TDB    ! dry-bulb temperature {C}
      REAL, intent(in) :: TWBin  ! wet-bulb temperature {C}
      REAL, intent(in) :: PB     ! barometric pressure {Pascals}
      character(len=*), intent(in), optional :: calledfrom  ! routine this function was called from (error messages)
      REAL        :: W      ! result=> humidity ratio
          ...
          ! FUNCTION LOCAL VARIABLE DECLARATIONS:
      REAL PWET  ! Pressure at wet-bulb temperature {Pascals}
      REAL WET   ! Humidity ratio at wet-bulb temperature
      REAL TWB   ! test wet-bulb temperature
...
!                                      VALIDITY CHECK.
      IF (W < 0.0) THEN
        ...
        ENDIF
#endif
        W=PsyWFnTdbRhPb(TDB,.0001,PB,calledfrom)
      ENDIF
...
FUNCTION PsyWFnTdbRhPb(TDB,RH,PB,calledfrom) RESULT(W)
  ...
  IMPLICIT NONE    ! Enforce explicit typing of all variables in this routine
      ...
      REAL        :: W      ! result=> humidity ratio
          ...
          ! FUNCTION LOCAL VARIABLE DECLARATIONS:
      REAL PDRY  ! Pressure at dry-bulb temperature {Pascals}
      REAL PDEW  ! Pressure at dew-point temperature {Pascals}
          ...
          IF (.not. WarmupFlag) THEN
            IF (iPsyErrIndex(iPsyWFnTdbRhPb) == 0) THEN
              String=' Dry-Bulb= '//TRIM(TrimSigDigits(TDB,2))//' Relative Humidity [%]= '//  &
                     TRIM(TrimSigDigits(RH*100.,2))//' Pressure= '//TRIM(TrimSigDigits(PB,2))
...
FUNCTION PsyPsatFnTemp(T,calledfrom) RESULT(Pascal)
  ...
  IMPLICIT NONE    ! Enforce explicit typing of all variables in this routine
          ...
          ! FUNCTION ARGUMENT DEFINITIONS:
      REAL, intent(in) :: T      ! dry-bulb temperature {C}
      character(len=*), intent(in), optional :: calledfrom  ! routine this function was called from (error messages)
      REAL        :: Pascal ! result=> saturation pressure {Pascals}

          ! FUNCTION PARAMETER DEFINITIONS:
      REAL, PARAMETER   :: C1 = -5674.5359      ! Coefficient for TKel < KelvinConvK
      REAL, PARAMETER   :: C2 =  6.3925247      ! Coefficient for TKel < KelvinConvK
      REAL, PARAMETER   :: C3 = -0.9677843d-2     ! Coefficient for TKel < KelvinConvK
      REAL, PARAMETER   :: C4 =  0.62215701d-6    ! Coefficient for TKel < KelvinConvK
      REAL, PARAMETER   :: C5 =  0.20747825d-8    ! Coefficient for TKel < KelvinConvK
      REAL, PARAMETER   :: C6 = -0.9484024d-12    ! Coefficient for TKel < KelvinConvK
      REAL, PARAMETER   :: C7 =  4.1635019      ! Coefficient for TKel < KelvinConvK

      REAL, PARAMETER   :: C8  =  -5800.2206    ! Coefficient for TKel >= KelvinConvK
      REAL, PARAMETER   :: C9  =   1.3914993    ! Coefficient for TKel >= KelvinConvK
      REAL, PARAMETER   :: C10 =  -0.048640239  ! Coefficient for TKel >= KelvinConvK
      REAL, PARAMETER   :: C11 =   0.41764768d-4  ! Coefficient for TKel >= KelvinConvK
      REAL, PARAMETER   :: C12 =  -0.14452093d-7  ! Coefficient for TKel >= KelvinConvK
      REAL, PARAMETER   :: C13 =   6.5459673    ! Coefficient for TKel >= KelvinConvK
      ...
      REAL, PARAMETER               :: KelvinConv = 273.15
          ...
          ! FUNCTION LOCAL VARIABLE DECLARATIONS:
      REAL   :: Tkel  ! Dry-bulb in REAL for function passing
      ...
      IF ((TKel .LT. KelvinConv) .AND. (TKel .GE. 173.15)) THEN
      ...
      ELSE IF ((TKel .Lt. 173.15)) THEN
         Pascal = 0.0017

       ! If above freezing, calculate saturation pressure over liquid water.
      ELSE IF ((TKel .GE. KelvinConv) .AND. (TKel .LE. 473.15)) THEN
...
!                                      TEMP. IS FROM -60 C  TO  -40 C
   30 CONTINUE
      T=F6(HH,-19.44,8.53675*10**-4,-5.12637*10**-9,-9.85546*10**-14,-1.00102*10**-18,-4.2705*10**-24)  !RS: Debugging: 102612
      GO TO 120
!                                      TEMP. IS FROM -40 C  TO  -20 C
   40 CONTINUE
      T=F6(HH,-1.94224*10,8.5892*10**-4,-4.50709*10**-9,-6.19492*10**-14,8.71734*10**-20,8.73051*10**-24)   !RS: Debugging: 102612
      GO TO 120
!                                      TEMP. IS FROM -20 C  TO    0 C
   50 CONTINUE
      T=F6(HH,-1.94224*10**1,8.59061*10**-4,-4.4875*10**-9,-5.76696*10**-14,7.72217*10**-19,3.97894*10**-24)
      GO TO 120
!                                      TEMP. IS FROM   0 C  TO   20 C
   60 CONTINUE
      T=F6(HH,-2.01147*10**1,9.04936*10**-4,-6.83305*10**-9,2.3261*10**-14,7.27237*10**-20,-6.31939*10**-25)
      GO TO 120
!                                      TEMP. IS FROM  20 C  TO   40 C
   70 CONTINUE
      T=F6(HH,-1.82124*10**1,8.31683*10**-4,-6.16461*10**-9,3.06411*10**-14,-8.60964*10**-20,1.03003*10**-25)
      GO TO 120
!                                      TEMP. IS FROM  40 C  TO   60 C
   80 CONTINUE
      T=F6(HH,-1.29419,3.88538*10**-4,-1.30237*10**-9,2.78254*10**-15,-3.27225*10**-21,1.60969*10**-27)
      GO TO 120
!                                      TEMP. IS FROM   60 C TO   80 C
   90 CONTINUE
      T=F6(HH,2.39214,1.27519*10**-4,-1.52089*10**-10,1.1043*10**-16,-4.33919*10**-23,7.05296*10**-30)
      GO TO 120
!                                      TEMP. IS FROM   80 C TO   90 C
  100 CONTINUE
      T=F6(HH,4.884461,3.85534*10**-5,-1.78805*10**-11,4.87224*10**-18,-7.15283*10**-25,4.36246*10**-32)
      GO TO 120
!                                      TEMP. IS FROM   90 C TO  100C
  110 CONTINUE
      T=F7(HH,7.60565*10**11,5.80534*10**4,-7.36433*10**-3,5.11531*10**-10,-1.93619*10**-17,3.70511*10**-25, -2.77313*10**-33)
...
!                                      FOLLOWING ITERATION METHOD
  120 CONTINUE
      ...
      ENDIF
#endif
      IF (ABS(PB-1.0133*10**5)/1.0133*10**5 <= 0.01) GO TO 170
      IterCount=0
      T1=T
      H1=PsyHFnTdbW(T1,PsyWFnTdbTwbPb(T1,T1,PB))
      Y1=H1-Hloc
      IF (ABS(Y1/Hloc) <= 0.1*10**-4) GO TO 140
      T2=T1*0.9
  130 IterCount=IterCount+1
      H2=PsyHFnTdbW(T2,PsyWFnTdbTwbPb(T2,T2,PB))
      Y2=H2-Hloc
      IF (ABS(Y2/Hloc) <= 0.1*10**-4) GO TO 150
      IF (Y2 == Y1) GO TO 150
...
!   result is T
  ...
  CONTAINS
      ...
      F7=(A0+X*(A1+X*(A2+X*(A3+X*(A4+X*(A5+X*A6))))))/1.0*10**10
...
FUNCTION PsyTsatFnPb(Press,calledfrom) RESULT(Temp)
  ...
  IMPLICIT NONE    ! Enforce explicit typing of all variables in this routine
          ...
          ! FUNCTION ARGUMENT DEFINITIONS:
      REAL, intent(in) :: Press   ! barometric pressure {Pascals}
      character(len=*), intent(in), optional :: calledfrom  ! routine this function was called from (error messages)
      REAL        :: Temp    ! result=> saturation temperature {C}
      ...
      REAL :: convTol=0.0001
          ...
          ! FUNCTION LOCAL VARIABLE DECLARATIONS:
      LOGICAL FlagError     ! set when errors should be flagged
      REAL,SAVE:: Press_Save=-99999.
      REAL,SAVE:: tSat_Save =-99999.
      REAL:: tSat           ! Water temperature guess
      REAL:: pSat           ! Pressure corresponding to temp. guess
      REAL:: error          ! Deviation of dependent variable in iteration
      REAL:: X1             ! Previous value of independent variable in ITERATE
      REAL:: Y1             ! Previous value of dependent variable in ITERATE
      REAL:: ResultX        ! ResultX is the final Iteration result passed back to the calling routine
      Integer :: iter       ! Iteration counter
      Integer :: icvg       ! Iteration convergence flag
      ...
      FlagError=.false.
#ifdef EP_psych_errors
      IF (Press <= 0.0017 .or. Press >= 1555000.) THEN
        IF (.not. WarmupFlag) THEN
...
FUNCTION CPCW(Temperature,calledfrom) RESULT(SpecHeatCW)
      ...
      Implicit NONE
          ...
          ! FUNCTION ARGUMENT DEFINITIONS:
      REAL, intent(in) :: Temperature !unused1208
      character(len=*), intent(in), optional :: calledfrom  ! routine this function was called from (error messages) !unused1208
      REAL        :: SpecHeatCW  ! result
...
FUNCTION RhoH2O(TB,calledfrom) RESULT(RhoResult)
      ...
      IMPLICIT NONE
          ...
          ! FUNCTION ARGUMENT DEFINITIONS:
      REAL, INTENT(in) :: TB  ! Dry bulb temperature. {C}
      character(len=*), intent(in), optional :: calledfrom  ! routine this function was called from (error messages) !unused1208
      REAL        :: RhoResult
        ...
        RhoResult=1000.1207+8.3215874*10**-04*TB-4.929976*10**-03*TB**2+8.4791863*10**-06*TB**3

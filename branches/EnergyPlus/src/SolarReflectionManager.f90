...
TYPE,PUBLIC :: SolReflRecSurfData
  ...
  REAL,ALLOCATABLE,DIMENSION(:,:)   :: RecPt        ! Coordinates of receiving point on receiving surface in global CS (m)
  REAL,DIMENSION(3)                 :: NormVec      =0.0 ! Unit outward normal to receiving surface
  REAL                         :: ThetaNormVec =0.0 ! Azimuth of surface normal (radians)
  REAL                         :: PhiNormVec   =0.0 ! Altitude of surface normal (radians)
  INTEGER                           :: NumReflRays  =0   ! Number of rays from this receiving surface
  REAL,ALLOCATABLE,DIMENSION(:,:)   :: RayVec       ! Unit vector in direction of ray from receiving surface
  REAL,ALLOCATABLE,DIMENSION(:)     :: CosIncAngRay ! Cosine of angle between ray and receiving surface outward normal
  REAL,ALLOCATABLE,DIMENSION(:)     :: dOmegaRay    ! Delta solid angle associated with ray
  REAL,ALLOCATABLE,DIMENSION(:,:,:) :: HitPt        ! For each receiving point and ray, coords of hit point on obstruction
  ...
  REAL,ALLOCATABLE,DIMENSION(:,:)   :: HitPtSolRefl ! Beam-to-diffuse solar reflectance at hit point
  REAL,ALLOCATABLE,DIMENSION(:,:)   :: RecPtHitPtDis ! Distance from receiving point to hit point (m)
  REAL,ALLOCATABLE,DIMENSION(:,:,:) :: HitPtNormVec ! Hit point's surface normal unit vector pointing into hemisphere
...
SUBROUTINE InitSolReflRecSurf
  ...
  REAL :: RecVec(3)            ! First vertex of a receiving surface (m)
  REAL :: ObsVec(3)            ! A vertex of a candidate obstructing surface (m)
  REAL :: VecAB(3)             ! Vector from receiving surface vertex to obstruction surface vertex (m)
  REAL :: HitPt(3)             ! Hit point (m)
  REAL :: DotProd              ! Dot product of vectors (m2)
  INTEGER   :: RecPtNum             ! Receiving point number
!unused  REAL         :: SumX                 ! Sum of X (or Y or Z) coordinate values of a surface
!unused  REAL         :: SumY                 ! Sum of X (or Y or Z) coordinate values of a surface
!unused  REAL         :: SumZ                 ! Sum of X (or Y or Z) coordinate values of a surface
  REAL :: PhiSurf              ! Altitude of normal to receiving surface (radians)
  REAL :: ThetaSurf            ! Azimuth of normal to receiving surface (radians)
  REAL :: PhiMin               ! Minimum and maximum values of ray altitude angle (radians)
  REAL :: PhiMax               ! Minimum and maximum values of ray altitude angle (radians)
  REAL :: ThetaMin             ! Minimum and maximum values of ray azimuth angle (radians)
  REAL :: ThetaMax             ! Minimum and maximum values of ray azimuth angle (radians)
  REAL :: Phi                  ! Ray altitude angle, increment, sine, and cosine
  REAL :: DPhi                 ! Ray altitude angle, increment, sine, and cosine
  REAL :: SPhi                 ! Ray altitude angle, increment, sine, and cosine
  REAL :: CPhi                 ! Ray altitude angle, increment, sine, and cosine
  REAL :: Theta                ! Ray azimuth angle and increment
  REAL :: DTheta               ! Ray azimuth angle and increment
  INTEGER   :: IPhi                 ! Ray altitude angle and azimuth angle indices
  INTEGER   :: ITheta               ! Ray altitude angle and azimuth angle indices
!unused  REAL         :: APhi                 ! Intermediate variable
  INTEGER   :: RayNum               ! Ray number
  REAL :: URay(3)              ! Unit vector along ray pointing away from receiving surface
  REAL :: CosIncAngRay         ! Cosine of angle of incidence of ray on receiving surface
  REAL :: dOmega               ! Solid angle associated with a ray
  ...
  REAL :: NearestHitPt(3)      ! Nearest hit pit for a ray (m)
  ...
  REAL :: RecPt(3)             ! Receiving point (m)
  REAL :: RayVec(3)            ! Unit vector along ray
  REAL :: Vec1(3)              ! Vectors between hit surface vertices (m)
  REAL :: Vec2(3)              ! Vectors between hit surface vertices (m)
  REAL :: VNorm(3)             ! For a hit surface, unit normal vector pointing into the hemisphere
                                    ! containing the receiving point
  ...
  REAL :: HorDis               ! Distance between ground hit point and proj'n of receiving pt onto ground (m)
  REAL :: GroundHitPt(3)       ! Coordinates of ground hit point
!unused  REAL         :: ArgASin
  REAL :: ACosTanTan
  ...
  REAL :: VertexWt             ! Vertex weighting factor for calculating receiving points
...
!CR8251      IF(DotProd > 0.01) THEN  ! This obstructing-surface vertex is not behind receiving surface
      IF(DotProd > 1.*10**-6) THEN  ! This obstructing-surface vertex is not behind receiving surface
        ObsBehindRec = .FALSE.
        EXIT
      END IF
    END DO
  ...
  DO J = 1,NumRecPts
    ...
    DO K = 1,NumRecPts
      ...
      ELSE                     ! Receiving surface has 4 or more vertices
        VertexWt = 1./(2.*REAL(NumRecPts,r64))
        IF(K == J) VertexWt = (REAL(NumRecPts,r64)+1.)/(2.*REAL(NumRecPts,r64))
      END IF
  ...
  DO IPhi = 1,AltAngStepsForSolReflCalc
    ...
    DO ITheta = 1,AzimAngStepsForSolReflCalc
      ...
      IF (CosIncAngRay < 0.0) CYCLE ! Ray is behind receiving surface (although there shouldn't be any)
      RayNum = RayNum + 1
...
!unused  INTEGER   :: loop                 =0   ! DO loop index
  REAL :: SunVec(3)            =0.0 ! Unit vector to sun
  ...
  REAL :: ReflBmToDiffSolObs(MaxRecPts) ! Irradiance at a receiving point for
                                            ! beam solar diffusely reflected from obstructions, divided by
                                            ! beam normal irradiance
  REAL :: ReflBmToDiffSolGnd(MaxRecPts) ! Irradiance at a receiving point for
  ...
  REAL :: OriginThisRay(3)     =0.0 ! Origin point of a ray (m)
  REAL :: ObsHitPt(3)          =0.0 ! Hit point on obstruction (m)
  INTEGER   :: ObsSurfNum           =0   ! Obstruction surface number
  REAL :: CosIncBmAtHitPt      =0.0 ! Cosine of incidence angle of beam solar at hit point
  REAL :: CosIncBmAtHitPt2     =0.0 ! Cosine of incidence angle of beam solar at hit point,
                                         !  the mirrored shading surface
  REAL :: BmReflSolRadiance    =0.0 ! Solar radiance at hit point due to incident beam, divided
                                         !  by beam normal irradiance
  REAL :: dReflBeamToDiffSol   =0.0 ! Contribution to reflection factor at a receiving point
                                         !  from beam solar reflected from a hit point
  REAL :: SunLitFract          =0.0 ! Sunlit fraction
          ! FLOW:
    ...
    DO RecPtNum = 1, SolReflRecSurf(RecSurfNum)%NumRecPts
      ...
      DO RayNum = 1, SolReflRecSurf(RecSurfNum)%NumReflRays
        ...
        IF(HitPtSurfNum > 0) THEN
          ...
          !IF(Surface(HitPtSurfNum)%HeatTransSurf .AND. SunlitFract < 0.01) CYCLE
          IF(SunlitFract < 0.01) CYCLE
...
!          ObsSurfNum = SolReflRecSurf(RecSurfNum)%PossibleObsSurfNums(loop)
        ...
        !BmReflSolRadiance = MAX(0.0,CosIncBmAtHitPt)*SolReflRecSurf(RecSurfNum)%HitPtSolRefl(RecPtNum,RayNum)

        BmReflSolRadiance = CosIncBmAtHitPt * SolReflRecSurf(RecSurfNum)%HitPtSolRefl(RecPtNum,RayNum)

        IF(BmReflSolRadiance > 0.0) THEN
    ...
    ReflFacBmToDiffSolGnd(SurfNum,IHr) = MIN(0.5*(1.-Surface(SurfNum)%CosTilt),  &
...
SUBROUTINE CalcBeamSolSpecularReflFactors
  ...
  REAL         :: SunVec(3)            =0.0 ! Unit vector to sun
  REAL         :: SunVecMir(3)         =0.0 ! Unit vector to sun mirrored by a reflecting surface
  INTEGER           :: RecSurfNum           =0   ! Receiving surface number
  INTEGER           :: SurfNum              =0   ! Heat transfer surface number corresponding to RecSurfNum
  INTEGER           :: NumRecPts            =0   ! Number of receiving points on a receiving surface
  INTEGER           :: RecPtNum             =0   ! Receiving point number
  REAL         :: RecPt(3)             =0.0 ! Receiving point (m)
  REAL         :: HitPtRefl(3)         =0.0 ! Hit point on a reflecting surface (m)
  REAL         :: ReflBmToDiffSolObs(MaxRecPts) ! Irradiance at a receiving point for
                                            ! beam solar diffusely reflected from obstructions, divided by
                                            ! beam normal irradiance
!unused  INTEGER           :: RayNum               =0   ! Ray number
  ...
  REAL         :: HitPtObs(3)          =0.0 ! Hit point on obstruction (m)
  ...
  REAL         :: ReflNorm(3)          =0.0 ! Unit normal to reflecting surface
  REAL         :: ReflBmToBmSolObs(MaxRecPts) ! Irradiance at a receiving point for
                                            ! beam solar specularly reflected from obstructions, divided by
                                            ! beam normal irradiance
  REAL         :: ReflDistance         =0.0 ! Distance from receiving point to hit point on a reflecting surface (m)
  REAL         :: ObsDistance          =0.0 ! Distance from receiving point to hit point on an obstruction (m)
  REAL         :: SpecReflectance      =0.0 ! Specular reflectance of a reflecting surface
  INTEGER           :: ConstrNumRefl        =0   ! Construction number of a reflecting surface
  REAL         :: CosIncAngRefl        =0.0 ! Cosine of incidence angle of beam on reflecting surface
  REAL         :: CosIncAngRec         =0.0 ! Angle of incidence of reflected beam on receiving surface
  REAL         :: ReflFac              =0.0 ! Contribution to specular reflection factor
  REAL         :: ReflFacTimesCosIncSum(MaxRecPts) ! Sum of ReflFac times CosIncAngRefl
  REAL         :: CosIncWeighted       =0.0 ! Cosine of incidence angle on receiving surf weighted by reflection factor
    ...
    IF(SolReflRecSurf(RecSurfNum)%NumPossibleObs > 0) THEN
      ...
      DO loop = 1, SolReflRecSurf(RecSurfNum)%NumPossibleObs
        ...
        IF((Surface(ReflSurfNum)%Class == SurfaceClass_Window .AND. Surface(ReflSurfNum)%ExtSolar) .OR. &
            (Surface(ReflSurfNum)%ShadowSurfGlazingFrac > 0.0 .AND. &
            Surface(ReflSurfNum)%ShadowingSurf)) THEN
          ReflBmToBmSolObs = 0.0
          ReflFacTimesCosIncSum = 0.0
          ! Skip if window and not sunlit
          IF(Surface(ReflSurfNum)%Class == SurfaceClass_Window .AND. SunlitFrac(ReflSurfNum,IHr,1) < 0.01) CYCLE
          ...
          SunVecMir = SunVec - 2.0*DOT_PRODUCT(SunVec,ReflNorm)*ReflNorm
...
SUBROUTINE CalcSkySolDiffuseReflFactors
  ...
  REAL :: ReflSkySolObs(MaxRecPts) ! Irradiance at a receiving point for sky diffuse solar
                                    ! reflected from obstructions, divided by unobstructed
                                    ! sky diffuse horizontal irradiance
  REAL :: ReflSkySolGnd(MaxRecPts) ! Irradiance at a receiving point for sky diffuse solar
                                    ! reflected from ground, divided by unobstructed
                                    ! sky diffuse horizontal irradiance
  INTEGER   :: RayNum               =0 ! Ray number
  REAL :: HitPtRefl(3)         =0.0 ! Coordinates of hit point on obstruction or ground (m)
  INTEGER   :: IHitObs              =0 ! > 0 if obstruction is hit; otherwise = 0
  REAL :: HitPtObs(3)          =0.0 ! Hit point on an obstruction (m)
!unused  REAL         :: ObsHitPt(3)          =0.0 ! Hit point on obstruction (m)
  REAL :: dOmega               =0.0 ! Solid angle increment (steradians)
  REAL :: CosIncAngRayToSky            =0.0 ! Cosine of incidence angle on ground of ray to sky
  REAL :: SkyReflSolRadiance   =0.0 ! Reflected radiance at hit point divided by unobstructed
                                    !  sky diffuse horizontal irradiance
  REAL :: dReflSkySol          =0.0 ! Contribution to reflection factor at a receiving point
                                    !  from sky solar reflected from a hit point
  REAL :: Phi                  =0.0 ! Altitude angle and increment (radians)
  REAL :: DPhi                 =0.0 ! Altitude angle and increment (radians)
  REAL :: SPhi                 =0.0 ! Sine of Phi
  REAL :: CPhi                 =0.0 ! Cosine of Phi
  REAL :: Theta                =0.0 ! Azimuth angle (radians)
  REAL :: DTheta               =0.0 ! Azimuth increment (radians)
  INTEGER   :: IPhi                 =0 ! Altitude angle index
  INTEGER   :: ITheta               =0 ! Azimuth angle index
  REAL :: URay(3)              =0.0 ! Unit vector along ray from ground hit point
  REAL :: SurfVertToGndPt(3)   =0.0 ! Vector from a vertex of possible obstructing surface to ground
                                    !  hit point (m)
  REAL :: SurfVert(3)          =0.0 ! Surface vertex (m)
  REAL :: dReflSkyGnd          =0.0 ! Factor for ground radiance due to direct sky diffuse reflection
          ! FLOW:
...
DO RecSurfNum = 1,TotSolReflRecSurf
        ...
        DO IPhi = 1,(AltAngStepsForSolReflCalc/2)
          ...
          DO ITheta = 1,2*AzimAngStepsForSolReflCalc
            ...
            DO ObsSurfNum = 1, TotSurfaces
              ...
              IF(Surface(ObsSurfNum)%Tilt < 5.0) CYCLE
              IF(.NOT.Surface(ObsSurfNum)%ShadowingSurf) THEN
                IF(DOT_PRODUCT(URay,Surface(ObsSurfNum)%OutNormVec) >= 0.0) CYCLE
                ...
                IF(Surface(ObsSurfNum)%Tilt > 89.0 .AND. Surface(ObsSurfNum)%Tilt < 91.0) THEN
                  SurfVert = Surface(ObsSurfNum)%Vertex(2)
                  SurfVertToGndPt = HitPtRefl - SurfVert
                  IF(DOT_PRODUCT(SurfVertToGndPt,Surface(ObsSurfNum)%OutNormVec) < 0.0) CYCLE
                END IF
              END IF
  ...
  ! Do not allow ReflFacBmToDiffSolGnd to exceed the surface's unobstructed ground view factor
  ReflFacSkySolGnd(SurfNum) = MIN(0.5*(1.-Surface(SurfNum)%CosTilt),  &
...
SUBROUTINE PierceSurface(ISurf, R1, RN, IPIERC, CPhit)
  ...
  REAL, INTENT(OUT)       :: CPhit(3)   ! Point that ray along RN intersects plane of surface
...
!unused  REAL :: AA(3)                    ! AA(I) = A(N,I)
!unused  REAL :: CC(3)                    ! CC(I) = C(N,I)
...
!unused  REAL :: DOTAXCSN                 ! Dot product of vectors AXC and SN


  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:) :: V     ! Vertices of surfaces
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:) :: A     ! Vertex-to-vertex vectors; A(1,i) is from vertex 1 to 2, etc.
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:) :: C     ! Vectors from vertices to intersection point
  ...
  IF (ABS(F2) < 0.01) RETURN   ! Skip surfaces that are parallel to RN
 ...
 ELSE
    ...
    IF (DOT_PRODUCT(AXC,SN) < 0.0) RETURN  ! If at least one dot product is negative, intersection outside of surface

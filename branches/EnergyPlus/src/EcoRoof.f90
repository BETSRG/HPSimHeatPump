...
PRIVATE
  ...
  REAL :: CumRunoff = 0.0  ! Cumulative runoff, updated each time step (m) mult by roof area to get volume
  REAL :: CumET = 0.0      ! Cumulative evapotranspiration from soil and plants (m)
  REAL :: CumPrecip = 0.0
  REAL :: CumIrrigation = 0.0 ! Cumulative irrigation, updated each time step (m) mult by roof area to get volume
  REAL :: CurrentRunoff
  REAL :: CurrentET
  REAL :: CurrentPrecipitation  ! units of (m) per timestep
  REAL :: CurrentIrrigation     ! units of (m) per timestep

  REAL :: Tfold     ! leaf temperature from the previous time step
  REAL :: Tgold     ! ground temperature from the previous time step
...
!  USE DaylightingDevices,        ONLY: FindTDDPipe
  ...
  REAL   , INTENT(OUT)   :: TempExt            ! Exterior temperature boundary condidtion

  !SUBROUTINE PARAMETER DEFINITIONS:
  REAL, PARAMETER  :: Kv         = 0.4         ! Von Karmen's constant (source FASST)
  REAL, PARAMETER  :: rch        = 0.63        ! Turbulent Schimdt Number
  REAL, PARAMETER  :: Ks         = 0.2         ! W/m.k. Thermal Conductivity of soil
  REAL, PARAMETER  :: rche       = 0.71        ! Turbulent Prandtl Number
  REAL, PARAMETER  :: Rv         = 461.53      ! Gas Constant of Water Vapor J/kg K
  REAL, PARAMETER  :: Rair       = 0.286d3     ! Gas Constant of air J/Kg K
  REAL, PARAMETER  :: g1         = 9.81        ! Gravity. In m/sec^2.
  REAL, PARAMETER  :: Sigma      = 5.6697d-08  ! Stefan-Boltzmann constant W/m^2K^4
  REAL, PARAMETER  :: Cpa        = 1005.6      ! Specific heat of Water Vapor. (J/Kg.K)
...
!  INTEGER :: OSCScheduleIndex    ! Index number for OSC ConstTempSurfaceName
  ...
  REAL,SAVE :: LAI       = 0.2       ! Leaf area index
  REAL,SAVE :: epsilonf  = 0.95      ! Leaf Emisivity
  REAL,SAVE :: epsilong  = 0.95      ! Soil Emisivity
  REAL,SAVE :: Alphag    = 0.3       ! Ground Albedo
  REAL,SAVE :: Alphaf    = 0.2       ! Leaf Albedo (reflectivity to solar radiation)
  REAL :: e0        = 2.0       ! Windless lower limit of exchange coefficient (from FASST docs)
  REAL :: RH        =50.0       ! Relative humidity (%)
  REAL :: Pa        =101325.    ! Atmospheric Pressure (PA)
  REAL,SAVE :: Tg        =10.0       ! Ground Surface temperature C ***** FROM PREVIOUS TIME STEP
  REAL,SAVE :: Tf        =10.0       ! Leaf temperature C ***** FROM PREVIOUS TIME STEP
  REAL :: Tgk                   ! Ground temperature in Kelvin
  REAL,SAVE :: Zf        = 0.2       ! Height of plants (m)
...
! DJS
  REAL,SAVE :: Moisture         ! m^3/m^3.The moisture content in the soil is the value provided by a user
  REAL,SAVE :: MoistureResidual=0.05 ! m^3/m^3. Residual & maximum water contents are unique to each material.
                                ! See Frankenstein et al (2004b) for data.
  REAL,SAVE :: MoistureMax      =0.5 ! Maximum volumetric moisture content (porosity) m^3/m^3
  REAL,SAVE :: MeanRootMoisture  ! Mean value of root moisture m^3/m^3
  REAL,SAVE :: SoilThickness    =0.2 ! Soil thickness (m)
  REAL,SAVE :: StomatalResistanceMin ! s/m . ! Minimum stomatal resistance is unique for each veg. type.
  REAL :: f3               =1.0 ! As the value of gd for tall grass is 0, then f3 = 1
                                ! ECMWF 2002 CY25R1 report has gd=0.0 for all veg except trees where gd=0.03.

  REAL :: Ta                    ! current air temperature
  REAL :: Zog   = 0.001         ! Ground roughness length scale (m)
  REAL :: Za    = 2.0           ! Instrument height where atmospheric wind speed is measured (m)
  REAL :: Ws                    ! Wind Speed (m/s)
  REAL :: Waf                   ! Windspeed within canopy (m/s)

  REAL :: Latm                  ! Long Wave Radiation (W/m^2)
  REAL :: qaf                   ! mixing ratio of air near canopy

  REAL :: qg                    ! mixing ratio of air at surface.
  REAL,SAVE :: Lf        ! latent heat flux
  REAL,SAVE :: Vfluxf = 0.0    ! Water evapotr. rate associated with latent heat from vegetation [m/s]
  REAL :: RS                    ! shortwave radiation
  REAL :: Qsoil = 0.0           ! heat flux from the soil layer
...
!unused1208  REAL :: e
  ...
  REAL :: Rhoa
  REAL :: Tak
  REAL :: qa       ! mixing ratio of air
  REAL :: Tafk
  REAL :: Taf
  REAL :: Rhof
  REAL :: Rhoaf    ! Average air density
  REAL :: sigmaf
  REAL :: Zd       ! zero displacement height (m)
  REAL :: Zo       ! foliage roughness length (m)
  REAL :: Cfhn     ! transfer coefficient at near-neutral conditions
  REAL :: Cf       ! bulk Transfer coefficient, equation 10 page 6 (FASST).
  REAL, SAVE :: sheatf ! sensible heat flux coeff for foliage (W/m^2K)
  REAL, SAVE :: sensiblef !  sensible heat transfer TO foliage (W/m^2) DJS Jan 2011
  REAL :: ra       ! Aerodynamic Resistance

  REAL  :: f1inv  ! intermediate calculation variable
  REAL  :: f2inv ! intermediate calculation variable

  REAL :: f1       ! intermediate calculation variable
  REAL :: f2       ! intermediate calculation variable
  REAL :: r_s      ! Minimum Stomatal Resistance, specific to each plant
  REAL :: Mg       ! Surface soil moisture content m^3/m^3 (Moisture / MoistureMax)
  REAL :: dOne     ! intermediate calculation variable
  REAL :: esf      ! the saturation vapor pressure (Pa)
  REAL :: qsf      ! Saturation specific humidity at leaf temperature (qfsat)
  REAL :: Lef      ! Latent heat of vaporation at leaf surface temperature (J/kg)

  REAL :: Desf     ! Derivative of Saturation vapor pressure
  REAL :: dqf      ! Derivative of saturation specific humidity
  REAL :: dqg      ! this is given by Clausius-Clapeyron equation
  REAL :: esg      ! Saturation vapor pressure (Pa)
  REAL :: qsg      ! Saturation specific humidity(mixing ratio?) at ground surface temperature
  REAL :: Leg      ! Latent heat vaporization  at the ground temperature (J/kg)
  REAL :: Desg     ! derivative of esg Saturation vapor pressure(?)
  REAL :: F1temp   ! intermediate variable in computing flux from the soil layer
  REAL :: P1       ! intermediate variable in the equation for Tf
  REAL :: P2       ! intermediate variable in the equation for Tf and Tg
  REAL :: P3       ! intermediate variable in the equation for Tf and Tg
  REAL :: Rhog     ! Density of air at the soil surface temperature
  REAL :: Rhoag    ! Average density of air with respect to ground surface and air temperature
  REAL :: Rib      ! Richardson Number
  REAL :: Chng     ! bulk transfer coefficient near ground
  REAL :: Ce       ! bulk transfer coefficient (this is in fact Ceg in equation 28 main report)
  REAL :: Gammah   ! latent heat exchange stability correction factor
  REAL :: Chg      ! in fact it is the same as Ce (=Ceg) is transfer coefficient (but wot?)
  REAL, SAVE :: sheatg   ! intermediate calculation variable - sensible flux coef (W/m^2K for ground)
  REAL, SAVE :: sensibleg ! sensible heat flux TO ground (w/m^2) DJS Jan 2011
  REAL :: T3G      ! intermediate variable in the equation for Tg
  REAL :: T2G      ! intermediate variable in the equation for Tg
  REAL :: LeafTK ! the current leaf's temperature (Kelvin)
  REAL :: SoilTK ! the current soil's temperature (Kelvin)
  REAL :: Chne     ! is similar to near ground bulk transfer coefficient for latent heat flux (at neutral condition)
  REAL :: Tif      ! previous leaf temperature
  REAL :: rn       ! rn is the combined effect of both stomatal and aerodynamic resistances
                   ! in fact this is called r'' in the main report
  REAL,SAVE :: Lg  =0.0   ! latent heat flux from ground surface
  REAL,SAVE :: Vfluxg = 0.0  ! Water evapotr. rate associated with latent heat from ground surface [m/s]
  REAL :: T1G        ! intermediate variable in the equation for Tg
  REAL :: Qsoilpart1 ! intermediate variable for evaluating Qsoil (part without the unknown)
  REAL :: Qsoilpart2 ! intermediate variable for evaluating Qsoil (part coeff of the ground temperature)
...
!  INTEGER,EXTERNAL :: getnewunitnumber ! external function to return a new (unique) unit for ecoroof writing
  ...
  IF (WS < 2.0) THEN    ! Later we need to adjust for building roof height...
  ...
  Latm = 1.0 * sigma * 1.0* Surface(SurfNum)%ViewFactorGround*( (GroundTempKelvin)**4) &
          + 1.0 * sigma * 1.0*Surface(SurfNum)%ViewFactorSky*(SkyTempKelvin)**4
...
! DJS July 2007
  ...
  If (Surfnum .eq. FirstEcoSurf) then
    ...
    Tak = Ta + KelvinConv
     ...
     sigmaf = 0.9 - 0.7*exp(-0.75*LAI)  ! Fractional veg cover based on (2) from FASST TR-04-25
    ...
    eair = (RH/100.0)*611.2*EXP( 17.67*Ta/(Tak-29.65) )
    ...
    Tafk = (1.-sigmaf)*Tak + &
         sigmaf*( 0.3*Tak+0.6*(Tif+KelvinConv)+0.1*Tgk)
    ...
    Zd = 0.701 * Zf**0.979     ! Zero displacement height
    Zo = 0.131 * Zf**0.997     ! Foliage roughness length. (m) Source Ballick (1981)
    if (zo .lt. 0.02) zo = 0.02 ! limit based on p.7 TR-04-25 and Table 2

    !transfer coefficient at near-neutral condition Cfhn
    Cfhn = (Kv/LOG((Za - Zd)/Zo))**2   !Equation 12, page 7, FASST Model
    Waf = 0.83*Cfhn**0.5*sigmaf*Ws + (1.- sigmaf)*Ws ! Wind Speed inside foliage. Equation #6, FASST model
    ...
    esf = 611.2 * EXP(17.67*Tif/(Tif+KelvinConv-29.65) )
    ...
    f2 = 1./f2inv                     ! In dry areas f2 --> LARGE so that r_s --> LARGE
                                        ! and both rn and Latent flux --> 0.0
    f3 = 1./( EXP(-0.0*(esf-eair)) )  ! Note the 0.0 here is gd which has value of 0.0
    ...
    desf = 611.2*exp(17.67*(Tf/(Tf+KelvinConv-29.65))) *      &
    ( 17.67*Tf*(-1.0)*(Tf+KelvinConv-29.65)**(-2) + 17.67/(KelvinConv-29.65 + Tf))
    dqf = ((0.622* Pa)/(Pa - esf)**2)*Desf !Derivative of saturation specific humidity
    esg = 611.2*EXP( 17.67*( Tg/((Tg+KelvinConv)-29.65)))       !Pa saturation vapor pressure
    ...
    Leg = 1.91846d6*(Tgk/(Tgk-33.91))**2
    !Check to see if ice is sublimating or frost is forming.
    if (tgold .lt. 0.0 ) Leg= 2.838d6  ! per FASST documentation p.15 after eqn. 37.

    desg = 611.2*exp(17.67*(Tg/(Tg+KelvinConv-29.65))) *      &
       (17.67*Tg*(-1.0)*(Tg+KelvinConv-29.65)**(-2) + 17.67/(KelvinConv-29.65 + Tg))
    ...
    Rhoag = (Rhoa + Rhog)/2.
    Rib = 2.0*g1*Za*(Taf-Tg) / ( (Tafk+Tgk)*Waf**2 )   !Richardson Number
    ...
    Chg = Gammah* ( (1.-sigmaf)*Chng+sigmaf*Cfhn ) !
    ...
    Ce = gammah * ((1.-sigmaf)*Chne+sigmaf*Cfhn)   ! this is in fact Ceg in eq (28)
    ...
    qaf = ( (1.0-sigmaf)*qa+sigmaf*(0.3*qa+0.6*qsf*rn+0.1*qsg*Mg) )/&
        ( 1.0-sigmaf*(0.6*(1.-rn)+0.1*(1.0-Mg)) )
    ...
    P1 = sigmaf *( Rs * (1-Alphaf) + epsilonf * Latm ) &
      - 3.*sigmaf*epsilonf*epsilong*sigma*(SoilTK)**4/EpsilonOne &
      - 3.*( -sigmaf*epsilonf*sigma - sigmaf*epsilonf*epsilong*sigma/EpsilonOne )*(LeafTK)**4 &
      + sheatf*(1.-0.7*sigmaf)*(Ta+KelvinConv) + LAI *   &
       Rhoaf * Cf * Lef * Waf * rn * ((1. - 0.7*sigmaf)/done)*qa + LAI*Rhoaf*Cf*Lef &
      * Waf * rn *(((0.6 * sigmaf*rn) / done ) - 1.) *( qsf-(LeafTK)*dqf)+LAI   &
      * Rhoaf * Cf * Lef * Waf * rn * ((0.1*sigmaf*Mg)/done)*(qsg-(SoilTK)* dqg)
    P2 = 4.0*(sigmaf*epsilonf*epsilong*sigma)*(SoilTK)**(3)/EpsilonOne + 0.1*sigmaf*sheatf+LAI*Rhoaf*Cf*Lef &
     *Waf*rn*(0.1*sigmaf*Mg)/done *dqg
    P3 = 4.0* (-sigmaf*epsilonf*sigma - (sigmaf*epsilonf*sigma*epsilong)/EpsilonOne) * (LeafTK)**3 + (0.6 * sigmaf - 1.) &
       * sheatf + LAI * Rhoaf * Cf * Lef * Waf * rn * (((0.6 * sigmaf * rn) / done)-1.0) * dqf
    ...
    T1G = (1.- sigmaf) * (Rs * (1.- Alphag) + epsilong * Latm)-                     &
        (3.0 * (sigmaf * epsilonf * epsilong * sigma)  / EpsilonOne) *          &
        (LeafTK)**4 - 3.0*(-(1.0-sigmaf) * epsilong * sigma           &
        - sigmaf*epsilonf*epsilong*sigma/EpsilonOne)*(SoilTK)**4         &
        + sheatg*(1.-0.7*sigmaf)*(Ta+KelvinConv)+ Rhoag*Ce*Leg*Waf*Mg*((1. - 0.7*sigmaf)/dOne)*qa   &  !finished by T1G
        + Rhoag*Ce*Leg*Waf*Mg*(0.1*sigmaf*Mg/dOne - Mg)*(qsg-(SoilTK)*dqg)                 &
        + Rhoag*Ce*Leg*Waf*Mg*(0.6*sigmaf*rn/dOne)*(qsf-(LeafTK)*dqf)  + Qsoilpart1 + Qsoilpart2*(KelvinConv)

    T2G = 4.0*(-(1.0-sigmaf)*epsilong*sigma - sigmaf*epsilonf*epsilong*sigma/EpsilonOne)*(SoilTK)**3   &
      + (0.1*sigmaf-1.0)*sheatg + Rhoag*Ce*Leg*Waf*Mg*( 0.1*sigmaf*Mg/dOne - Mg )*dqg - Qsoilpart2

    T3G = ( 4.0*( sigmaf*epsilong*epsilonf*sigma )/EpsilonOne )*(LeafTK)**3 + 0.6*sigmaf*sheatg      &
...
SUBROUTINE UpdateSoilProps(Moisture,MeanRootMoisture,MoistureMax,MoistureResidual,SoilThickness,Vfluxf,Vfluxg,  &
  ...
  IMPLICIT NONE    ! Enforce explicit typing of all variables in this routine
          ...
          ! SUBROUTINE ARGUMENT DEFINITIONS:
  REAL, INTENT(INOUT) :: Moisture
  REAL, INTENT(INOUT):: MeanRootMoisture
  REAL, INTENT(IN)    :: MoistureMax
  REAL, INTENT(IN)    :: MoistureResidual
  REAL, INTENT(IN)    :: SoilThickness
  REAL, INTENT(IN)    :: Vfluxf    ! Water mass flux from vegetation [m/s]
  REAL, INTENT(IN)    :: Vfluxg    ! Water mass flux from soil surface [m/s]
  INTEGER, INTENT(INOUT) :: ConstrNum          ! Indicator for contruction index for the current surface
  REAL, INTENT(INOUT) :: Alphag
  INTEGER, INTENT(IN) :: unit    !unused1208
  REAL, INTENT(IN)    :: Tg !unused1208
  REAL, INTENT(IN)    :: Tf !unused1208
  REAL, INTENT(IN)    :: Qsoil !unused1208

          ! SUBROUTINE PARAMETER DEFINITIONS:
          ! na

  !Soil Parameters from Reference listed in the code:
  REAL, PARAMETER :: alpha=23   !These parameters are empirical constants
  REAL, PARAMETER :: n=1.27     !These parameters are empirical constants
  REAL, PARAMETER :: lambda=0.5 !These parameters are empirical constants
  !This is another parameter of the soil which describes the soil conductivity at the saturation point (m/s)
  REAL, PARAMETER ::SoilConductivitySaturation=5.157d-7
  ...
  REAL :: RatioMax
  REAL :: RatioMin
  REAL :: MoistureDiffusion ! moisture transport down from near-surface to root zone
  REAL,SAVE :: TopDepth  ! Thickness of "near-surface" soil layer
  REAL,SAVE :: RootDepth ! Thickness of "root zone" soil layer
                    ! Note TopDepth+RootDepth = thickness of ecoroof soil layer
  REAL,SAVE :: SecondsPerTimeStep  ! Seconds per TimeStep
  REAL :: SoilConductivity ! Moisture dependent conductivity to be fed back into CTF Calculator
  REAL :: SoilSpecHeat ! Moisture dependent Spec. Heat to be fed back into CTF Calculator
  REAL :: SoilAbsorpSolar  ! Moisture dependent Solar absorptance (1-albedo)
  REAL :: SoilDensity      ! Moisture dependent density to be fed back into CTF Calculator

  REAL :: SatRatio
  REAL :: TestRatio        ! Ratio to determine if timestep change in properties is too abrupt for CTF

  REAL,SAVE :: DryCond         ! Dry soil value of conductivity
  REAL,SAVE :: DryDens         ! Dry soil value of density
  REAL,SAVE :: DryAbsorp       ! Dry soil value of solar absorptance (1-albedo)
  REAL,SAVE :: DrySpecHeat     ! Dry soil value of specific heat
  REAL :: AvgMoisture     ! Average soil moisture over depth of ecoroof media

  LOGICAL,SAVE  :: UpdatebeginFlag = .TRUE.  ! one time flag

  REAL, SAVE ::CapillaryPotentialTop=-3.8997 !This variable keeps track of the capillary potential of the soil in both layers and time (m)
  REAL, SAVE ::CapillaryPotentialRoot=-3.8997 !This variable keeps track of the capillary potential of the soil in both layers and time (m)
  REAL, SAVE ::SoilHydroConductivityTop=8.72d-6 !This is the soil water conductivity in the soil (m/s)
  REAL, SAVE ::SoilHydroConductivityRoot=8.72d-6 !This is the soil water conductivity in the soil (m/s)
  REAL, SAVE ::SoilConductivityAveTop=8.72d-6 !This is the average soil water conductivity (m/s)
  REAL, SAVE ::SoilConductivityAveRoot=8.72d-6
  REAL, SAVE ::RelativeSoilSaturationTop !Relative Soil Saturation (soil moisture-residual soil moisture)/(saturation soil moisture-residual soil moisture)
  REAL, SAVE ::RelativeSoilSaturationRoot
  REAL ::TestMoisture=0.15 !This makes sure that the moisture cannot change by too much in each step
  INTEGER :: index1
  INTEGER :: ErrIndex=0
  ...
  If (UpdatebeginFlag) then
    ...
    IF (Material(Construct(ConstrNum)%LayerPoint(1))%EcoRoofCalculationMethod == 2) THEN
      DO index1=1,20,1
        IF ((REAL(MinutesPerTimeStep/index1,r64)) .LE. (161240.*2.**(-2.3)*(TopDepth+RootDepth)**(2.07))/60.) EXIT
      END DO
      IF (index1 .GT. 1) THEN
...
!      CALL ShowFatalError('Program terminates due to previous condition.')
  ...
  Moisture = Moisture - (Vfluxg)*MinutesPerTimeStep*60.0/TopDepth  ! soil surface evaporation
  MeanRootMoisture = MeanRootMoisture - (Vfluxf)*MinutesPerTimeStep*60.0/RootDepth ! plant extraction from root zone
...
!    elseif (Irrigation%ModeID ==IrrSmartSched .and. moisture .lt. 0.4*MoistureMax) then
 ...
 ! soil still gets the same total moisture... it is just distributed over a longer period.
    if (CurrentIrrigation + CurrentPrecipitation .gt. 0.5*0.0254*MinutesPerTimeStep/60.0) then
  ...
  ! it can be brought up.
    ...
    ! move moisture from top layer into root zone
      ...
      MoistureDiffusion= 0.00005*MinutesPerTimeStep*60.0*MoistureDiffusion
    ...
    ! at this point moistureDiffusion is in units of (m)/timestep
      MoistureDiffusion= 0.00001*MinutesPerTimeStep*60.0*MoistureDiffusion
      Moisture = Moisture + MoistureDiffusion/TopDepth
      MeanRootMoisture = MeanRootMoisture - MoistureDiffusion/RootDepth
    endif
  ELSE
  ...
  !Now, solve for the soil parameters for  of the top soil layer
    ...
    IF (RelativeSoilSaturationTop < 0.0001) THEN
      IF (ErrIndex == 0) THEN
    ...
    SoilHydroConductivityTop=SoilConductivitySaturation*(RelativeSoilSaturationTop**lambda)*  &
       (1.-(1.-(RelativeSoilSaturationTop)**(n/(n-1.)))**((n-1.)/n))**2
    CapillaryPotentialTop=(-1./alpha)*((((1./RelativeSoilSaturationTop)**(n/(n-1.)))-1.)**(1./n))
    ...
    SoilHydroConductivityRoot=SoilConductivitySaturation*(RelativeSoilSaturationRoot**lambda)*  &
       (1.-(1.-(RelativeSoilSaturationRoot)**(n/(n-1.)))**((n-1.)/n))**2
    CapillaryPotentialRoot=(-1./alpha)*((((1./RelativeSoilSaturationRoot)**(n/(n-1.)))-1.)**(1./n))
    ...
    IF(Moisture .GE. MoistureMax) THEN !This statement makes sure that the top layer is not over the moisture maximum for the soil.
         ...
         CurrentRunOff=CurrentRunOff+(Moisture-MoistureMax*0.9999)*TopDepth
    END IF

    !Now make sure that the soil does not go below the moisture minimum
    IF(Moisture .LE. (1.01*MoistureResidual)) THEN
    ...
    IF((SoilConductivityAveRoot*3600.) .LE. (2.33d-7)) THEN
  ...
  if (MeanRootMoisture .LE. MoistureResidual*1.00001) then
    Moisture = Moisture - (MoistureResidual*1.00001 - MeanRootMoisture)*RootDepth/TopDepth
    ...
    if (Moisture .lt. MoistureResidual*1.00001) Moisture = MoistureResidual*1.00001
  ...
  if (SoilAbsorpSolar .GT. 0.95 ) SoilAbsorpSolar = 0.95
  ! Limit solar absorptivity to greater than 20% so that albedo is always less than 80%
  if (SoilAbsorpSolar .LT. 0.20) SoilAbsorpSolar = 0.20
...
! OLD ::  SoilSpecHeat = DrySpecHeat*(1.0+ 0.4*(AvgMoisture-MoistureResidual)/(MoistureMax-MoistureResidual))
  ...
  ! Note wet soil has thermal conductivity that is up to 3 times that of  dry soil ...
  ! OLD :: SoilConductivity = DryCond* (1.0 + 1.0 * (AvgMoisture-MoistureResidual)/(MoistureMax-MoistureResidual))
  ! This is now based on Melos Hagos's results for k/kdry (March 2009)
    ...
    SoilConductivity = (DryCond/1.15) * (1.45 * exp(4.411 * SatRatio))/(1.0 + 0.45 * exp(4.411 * SatRatio))
